...
<div class="space-y-4">
	<section class="app-content-card px-5 py-5 md:px-6">
		...
		<div class="metric-grid mt-6">
			<div class="surface-panel-muted p-4">
				<p class="text-xs font-semibold tracking-[0.18em] text-base-content/55 uppercase">Plan</p>
			...
			<div class="surface-panel-muted p-4">
				<p class="text-xs font-semibold tracking-[0.18em] text-base-content/55 uppercase">Status</p>
				<p class="mt-3 flex items-center gap-2 text-sm font-medium">
	...
	>
		<FormAlert message={data.notice ?? form?.message} tone={data.notice ? 'info' : 'error'} />

		<div class="metric-grid md:grid-cols-2 xl:grid-cols-4">
			<div class="surface-panel-muted p-5">
				<p class="text-xs font-semibold tracking-[0.18em] text-base-content/60 uppercase">Plan</p>
				<p class="mt-3 text-2xl font-semibold">{data.billing.currentPlan.name}</p>
			</div>
			<div class="surface-panel-muted p-5">
				<p class="text-xs font-semibold tracking-[0.18em] text-base-content/60 uppercase">Status</p>
			...
			<div class="surface-panel-muted p-5">
				...
				<p class="mt-3 text-sm leading-6">
					{data.billing.allowsTeamInvites ? 'Included on this plan' : 'Upgrade to invite teammates'}
				</p>
			</div>
		</div>
		...
		<div class="surface-panel-muted flex flex-wrap items-center justify-between gap-3 p-4">
			...
			{#if data.billing.customerPortalAvailable}
				<form method="POST" action="?/portal" use:enhance>
					<button class="btn rounded-full btn-outline" type="submit">Open billing portal</button>
				</form>
			{/if}
		</div>
	</SectionCard>
	...
	>
		<div class="grid gap-4 xl:grid-cols-3">
			{#each data.billing.plans as plan (plan.key)}
				<div
					class={`surface-panel flex h-full flex-col gap-4 p-6 ${plan.highlighted ? 'shadow-[0_18px_60px_-30px_rgba(19,128,103,0.35)] ring-1 ring-primary/30' : ''}`}
				>
					<div class="flex items-start justify-between gap-3">
						<div>
							...
							<p class="mt-3 text-3xl font-semibold">{plan.priceLabel}</p>
						</div>
						{#if plan.isCurrent}
					...
					<div class="mt-auto pt-2">
						...
						{:else if plan.canStartCheckout}
							<form method="POST" action="?/checkout" use:enhance>
								...
								<button
									class={`btn w-full rounded-full ${plan.highlighted ? 'btn-primary' : 'btn-outline'}`}
									type="submit"
								>
									{plan.ctaLabel}
								</button>
